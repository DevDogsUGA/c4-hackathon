...
Open `bot.go` and change the `chooseMove(board, you, info)` function. That's
it. `server.go` is the HTTP server (CORS headers, JSON parsing, error
...
- `info` (`MoveInfo`, defined in `bot.go`) is extra context you can ignore:
  `Moves` (the move history), `MatchID`, `GameNumber`, and
