...
Open `bot.py` and change the `choose_move(board, you, info=None)` function.
That's it. `server.py` is the HTTP server (CORS headers, JSON parsing, error
...
- `info` is optional context you can ignore. When passed, it's an object
  with `.moves` (the move history), `.match_id`, `.game_number`, and
  `.clock_remaining_ms` (your remaining think-time budget for **this game**,
  not this move). You don't need to import anything to use or ignore it.
